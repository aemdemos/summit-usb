...
**`footer.plain.html` contract:** One fragment for **local preview and DA/EDS** — same rules everywhere. The file holds **portable semantic content** (headings, paragraphs, lists, links, images) that plain processing accepts. **Do not** put form controls (`<form>`, `<input>`, `<button>`, `<label>`) or layout that depends on `class` / `data-*` / `style` / `id` on wrappers inside the fragment; Document Authoring and other pipelines strip or flatten those, and relying on “GitHub-only” HTML in plain.html breaks portability. Build controls and layout shells in `footer.js`, style them in `footer.css`.

**CRITICAL RULE:** All **copy and media** that can live in the fragment (text, links, images, locale entries as plain-friendly markup) MUST be in `footer.plain.html`. `footer.js` reads that DOM — do **not** hardcode strings that duplicate fragment content. The hook blocks hardcoded content patterns.
...
- **CRITICAL — DA/EDS flat structure (`content/footer.plain.html`):** The fragment must stay **flat and semantic**: only **top-level section `<div>`s** (no nested layout `<div>`s inside a section); **no** `<form>`, `<input>`, `<button>`, `<label>`, `<select>`, or `<textarea>`; **no** `class=`, `data-*`, `style=`, or `id=` on markup in the fragment — put layout, controls, and selectors in **`footer.js`** + **`footer.css`** after fetch. This is enforced by the PostToolUse **`FLAT_STRUCTURE`** gate and by **`validate-footer-content.js`**; both must pass (see **`footer.plain.html` contract** and `references/element-handling-guide.md`).
...
5. **Write** `blocks/footer/footer.js` — reads from DOM, renders sections. NEVER hardcode *copy* (use the fetched fragment as source of truth where allowed). **Dual-fetch (this order is correct):** (1) `fetch('/content/footer.plain.html')` — localhost / aem up; (2) if not OK, `fetch` with `` `${footerPath}.plain.html` `` — DA/EDS production. Set `footerPath` from block metadata (path to the footer doc **without** the `.plain.html` suffix).
