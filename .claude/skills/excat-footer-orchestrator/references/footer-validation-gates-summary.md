...
## PostToolUse Gates (run on every Write/Edit) — 13 total
...
| FLAT_STRUCTURE | `content/footer.plain.html` | block | No nested section `<div>`s; no form/input/button/label/select/textarea; no `class` / `data-*` / `style` / `id` on fragment markup (`checkFooterContentFlatStructure` + `validate-footer-content.js`) |
