...
const FLAT_STRUCTURE_LOG_MAX = 12_000;
...
export function runPostToolUseGates(ctx, gates, log) {
  ...
  for (const gate of gates) {
    ...
    if (!result.pass) {
      ...
      const fullMsg = result.message || '';
      const forLog =
        gate.id === 'FLAT_STRUCTURE'
          ? (fullMsg.length > FLAT_STRUCTURE_LOG_MAX ? `${fullMsg.slice(0, FLAT_STRUCTURE_LOG_MAX)}…` : fullMsg)
          : fullMsg.length > 200
            ? `${fullMsg.slice(0, 200)}...`
            : fullMsg;
      log(severity === 'warn' ? 'WARN' : 'BLOCK', `Gate ${gate.id} ${severity === 'warn' ? 'WARNING' : 'FAILED'}`, forLog ? [forLog] : null);
      return { decision: severity, reason: result.message };
    }
    if (gate.id === 'FLAT_STRUCTURE') {
      log('PASS', 'Gate FLAT_STRUCTURE: OK — fragment passes flat semantic structure (no nested section divs; no forbidden tags/attrs)');
    }
  }
...
export const POST_TOOL_USE_GATES = [
  ...
  {
    id: 'FLAT_STRUCTURE',
    when: (ctx) => isFooterContentFile(ctx.filePath),
    severity: 'block',
    check: (ctx) => {
      const err = checkFooterContentFlatStructure(ctx.filePath);
      if (err) return { pass: false, message: `🚫 [Footer Gate] ${err}` };
      return { pass: true };
    },
  },
