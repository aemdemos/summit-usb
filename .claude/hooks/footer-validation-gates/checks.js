...
import { checkFlatSemanticStructure } from '../html-utils.js';
...
/**
 * PostToolUse: flat semantic structure for content/footer.plain.html.
 * Returns null (pass) if not footer content file path or unreadable; otherwise error string or null if clean.
 */
export function checkFooterContentFlatStructure(filePath) {
  const base = path.basename(filePath || '');
  if (base !== 'footer.plain.html') return null;
  let html = '';
  try {
  ...
  const { failures } = checkFlatSemanticStructure(html, { jsBlockName: 'footer.js' });
  if (failures.length === 0) return null;
  return `FLAT_STRUCTURE — content/footer.plain.html must be DA/EDS-safe:\n\n${failures.map((f) => `  • ${f}`).join('\n')}`;
}
