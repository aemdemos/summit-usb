 ...
 *
 * LOGGING: Writes to BOTH /tmp (always) and workspace debug.log.
 * When editing footer / validation / block files, the validation dir is created if missing so `debug.log` is always written.
 * PostToolUse **FLAT_STRUCTURE** logs full remediation (up to 12k chars) in the gate log; pass/fail + Stop hook re-check.
...
import {
  ...
  checkFooterContentFlatStructure,
...
function initWorkspaceLog(workspaceRoot, options = {}) {
  const { ensureDir = false } = options;
  const logDir = path.join(workspaceRoot, VALIDATION_DIR);
  if (ensureDir && !fs.existsSync(logDir)) {
    try {
...
function handlePostToolUse(hookInput) {
  ...
  if (!workspaceDebugLog && (isFooterContentFile(filePath) || isFooterValidationFile(filePath) || isFooterBlockFile(filePath))) {
    initWorkspaceLog(workspaceRoot, { ensureDir: true });
  }
  ...
  if (result.decision === 'block') {
    ...
    if (reason.includes('FLAT_STRUCTURE')) {
      log('BLOCK', '[PostToolUse] BLOCK — FLAT_STRUCTURE — write to footer.plain.html rejected (full detail in Gate FLAT_STRUCTURE FAILED log entry above)');
    }
    logDecision('block', result.reason?.slice(0, 120) || 'Gate blocked');
...
function handleStop(hookInput) {
  ...
  if (!footerFilePath) {
    ...
    const flatErr = checkFooterContentFlatStructure(footerFilePath);
    if (flatErr) {
      allErrors.push(flatErr);
      log('INFO', '[FLAT_STRUCTURE] Stop check: footer.plain.html fails flat semantic structure', { detail: flatErr.slice(0, 800) });
    }
